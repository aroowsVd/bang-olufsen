...
function full_sub_sizing(){
    ...
    $(".inner_dots > li").each(function() {
        $(this).click(function() {
            let btnSecIdx = $(this).parent().parent().index();
            let btnIdx = $(this).index();
            sub_counter = btnIdx + 1;
            move_w = prnts_w;
            move_w = move_w * btnIdx * -1;
            $(this).parent().find("li").removeClass("selected");
            $(this).addClass("selected");
            $(".fullsection").eq(btnSecIdx).find(".full_sub_con").stop().animate({left: move_w}, change_speed).attr("data-index", sub_counter).find(".full_sub").eq(sub_counter-1).removeClass("hide");
        });
    });
}
$(function(){
